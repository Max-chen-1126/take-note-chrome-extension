...
### 規劃：Cloud Run 內建 IAP（post-0.1.0 強化）
> 以下為 post-0.1.0 規劃中的邊緣認證強化方案，尚未實作；保留設計供未來切換參考。
...
> ⚠️ 此 IAP↔extension 程式化路徑切換前須先以 spike 驗證（見 Phase B 計畫 Task 1），驗證通過前繼續沿用 0.1.0 app 層 auth。
