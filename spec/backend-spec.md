...
| Auth | 0.1.0：app 層（後端驗 ID token 簽章 + aud==OAUTH_CLIENT_ID + email allowlist；Cloud Run --allow-unauthenticated）。規劃（post-0.1.0）：Cloud Run 內建 IAP。|
...
- **0.1.0 範圍**：Gemini-only、app 層認證（ID token 簽章 + aud + allowlist；IAP 留待 post-0.1.0）、四類筆記模板（youtube/blog/podcast/coursera）+ 共用 global-style，產物含可貼進 Obsidian 的 YAML frontmatter。
...
Extension ──POST /notes/stream (ID token, SSE)──▶ FastAPI (Cloud Run, asia-east1)
                                                   │
   後端 middleware：驗 ID token 簽章 + aud==OAUTH_CLIENT_ID + email allowlist（規劃：改用 Cloud Run IAP 邊緣認證）
...
## 8. Auth & 認證流程
- 0.1.0 採**app 層認證**：extension 用 Web OAuth client 經 `chrome.identity.launchWebAuthFlow` 取得 Google ID token（`aud` = 該 OAuth client_id），請求帶 `Authorization: Bearer <id_token>`。
- 後端 `verify_request`：驗 ID token 簽章 → 檢查 `aud == OAUTH_CLIENT_ID`（`settings.oauth_client_id`）→ email 比對 allowlist。缺/壞 token → `401`；email 不在白名單 → `403`。
- Cloud Run 部署 `--allow-unauthenticated`（邊緣對外開放，實際保護落在 app 層 token 驗證 + allowlist）。

### IAP 遷移評估：已驗證為結構性不可行，維持 app 層認證
> post-0.1.0 曾規劃改用 Cloud Run 內建 IAP 作為邊緣認證強化方案；此方案已實際 spike 驗證過（非僅紙上規劃），結論為**結構性死路（no-org 專案），非程式碼或設定問題，不再重試、不再規劃**。
- Spike 實作：於 `max-personal-447802` 對 Cloud Run 服務啟用內建 IAP（`--iap`）；extension 端登入流程正常運作（custom OAuth client 的 redirect URI 已註冊、`launchWebAuthFlow` 能正常取得 ID token），但 IAP edge 拒絕該 token，回應 `Invalid JWT audience`。
- 根因：IAP 的 `programmatic_clients` 允許清單機制要求該 OAuth client 必須隸屬於一個**組織（organization）**；實際執行 `gcloud iap settings set` 加入允許清單時失敗，錯誤訊息為「Programmatic Oauth Client ... does not belong to an organization」。專案 `max-personal-447802` 是 no-org 個人專案，因此 IAP 程式化存取路徑對此專案而言**結構性不可行**——無法靠重試、換一種 OAuth client 設定或改寫後端程式碼解決。
- 已還原現狀：執行 `gcloud run services update --no-iap` 關閉 IAP，並將 `allUsers` 重新加回 `roles/run.invoker`。驗證結果：請求不再帶 `x-goog-iap` 系列 header、`/methodologies` 回 `200`、`/notes/stream` 未帶 token 回 `401`——確認實際運作中的是本節開頭所述的 app 層認證鏈（token `aud` 驗證 + email allowlist），而非 IAP。
- **決定**：專案維持 0.1.0 的 app 層認證（ID token 簽章 + `aud == OAUTH_CLIENT_ID` + email allowlist），即目前實際部署運行中的機制。此 IAP 邊緣認證方案評估**結案，不再列入規劃**。
...
env:
  ...
  OAUTH_CLIENT_ID: <Web OAuth client_id>   # app 層認證 aud 驗證用
  ALLOWED_EMAILS: "maxwellchen1126@gmail.com"   # 逗號分隔
  CLOUD_RUN_SERVICE_URL: <deploy 後填>  # 0.1.0 app 層不使用（IAP 規劃時亦不需）
...
- custom OAuth client（Web 應用程式類型）於 GCP Console 建立並取得 client_id，填入 `OAUTH_CLIENT_ID`（非 secret，可放 env）。
...
- `gcloud run deploy take-note-backend --region asia-east1 --allow-unauthenticated --timeout=3600 --max-instances=1 --concurrency=8 --service-account <sa> --set-env-vars OAUTH_CLIENT_ID=...,ALLOWED_EMAILS=...,GOOGLE_CLOUD_PROJECT=max-personal-447802,GOOGLE_GENAI_USE_VERTEXAI=TRUE,GOOGLE_CLOUD_LOCATION=global`
